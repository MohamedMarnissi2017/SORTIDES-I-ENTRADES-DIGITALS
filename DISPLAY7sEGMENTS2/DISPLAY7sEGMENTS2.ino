...
void setup() {                 // configura el final de salida
  ...
  for ( int i = 0; i < 7; i++)//inicia funcio for, estableix variable i amb un valor de 0 , condiciona, si i es inferior a 8 incrementa el valor de i en 1.
  {
    ...
    pinMode(entrada, INPUT);// estableix totes les variables de l'array LEDS com a sortides
}
//*************************** LOOP ******************************
...
while(op==1)
{
  ...
  for (int i = 0; i < 7; i++) //inicia funcio for, estableix variable i amb un valor de 0 , condiciona, si i es inferior a 7 incrementa el valor de i en 1.
  {
    digitalWrite(LEDS[i],N9[i]);//Escriu valor de sortida de l'array LEDS en funcio de l'array N0
   delay(10);
    if(digitalRead(9)==HIGH)
  {
    op=op+1;
...
for (int i = 0; i < 7; i++) //inicia funcio for, estableix variable i amb un valor de 0 , condiciona, si i es inferior a 7 incrementa el valor de i en 1.
  {
    digitalWrite(LEDS[i],LOW);//Escriu valor de sortida de l'array LEDS en funcio de l'array N0
  }
  op=0;
  }
}
